...
<head>
	<title><?php
		$route = Merlot\App::resolve( 'routes' )->currentRoute();
		$paged = '';
		$title_tag = 'p';

		if ( false !== strpos( $route, 'page/{number}' ) ) {
			$p = explode( '/', Merlot\App::resolve( 'request' )->uri() );
...
<?php endif;
	...
	<meta name="twitter:text:title" content="<?= ! empty( $title ) ? e( $title ) : 'Benjamin Lu' ?>" />
	<link rel="alternate" type="application/rss+xml" title="Benjamin Lu Feed" href="http://feeds.feedburner.com/JustinTadlock" />
</head>
<body>
...
<div class="app">
	...
	<header class="app-header">
		<<?= e( $title_tag ) ?> class="app-header__title">
			<a class="app-header__title-link" href="<?= e( uri() ) ?>">Benjamin Lu</a>
		</<?= e( $title_tag ) ?>>
